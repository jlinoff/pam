...
5. **Done — 15 shots, 4 images retired.** Multi-step flows. Each is a scripted
   walk through a dialogue, so the captures came cheaply once the walk was
   ...
   **visibility**, not presence — presence passes in both states and proves
   ...
   Five of the nineteen were annotated. Four of those turned out to be pictures
   the README already had, once the arrows became prose: `pam-change-field-name`
   is `pam-fld-name-edit-on`, `pam-create-new-record` and `pam-new-record-menu`
   are both `pam-menu`, and `pam-clone-google` is `pam-google-record`. Only
   `pam-fld-name-edit-on` needed capturing.
