...
import { copyTextToClipboard, hide, show } from './utils.js'

export function enableRawJSONEdit() {
    let id = 'x-edit-raw-json-data'
    ...
    let topDiv = xmk('div').xId('x-edit-raw-data-div')
        ...
        .xAppend(
            ...
            xmk('div').xAppend(
                ...
                xmk('button')
                    .xClass('btn', 'btn-secondary', 'm-1')
                    .xAttrs({'title': 'copy data to clipboard'})
                    .xInnerHTML('Copy')
                    .xAddEventListener('click', (event) => { /* jshint ignore:line */
                        let ta = document.getElementById('x-edit-raw-data-textarea')
                        ta.focus()
                        copyTextToClipboard(ta.value)
                        statusBlip(`{ta.value} bytes copied to the clipboard`)
                    }),
            ),
