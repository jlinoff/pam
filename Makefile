...
#
# PORT is threaded through to the tests themselves, not just the server. It was
# not until v2.4.0: the Makefile started a server on $(PORT) while the tests
# talked to a hardcoded 8081, so a non-default port silently tested nothing.
#
# Two targets can therefore run at once on different ports, e.g.
#     make test &
#     make screenshots PORT=8082
# The only other shared state is www/js/version.js, which is written
...
unit-test: init lint ## Run vanilla JS unit tests in tests/tests.html via ChromeDriver
	...
	sleep 2
	lsof -i :$(PORT)
	PORT=$(PORT) pipenv run python3 -m pytest -v -s tests/test_unit.py
	$(KILL_SERVER)
...
screenshots: init ## Capture README screenshots. SHOT=<substr> limits the set.
	...
	PORT=$(PORT) CHECK=0 SHOT="$(SHOT)" pipenv run python3 -u tests/screenshots.py
	$(KILL_SERVER)
...
screenshots-check: init ## Report stale screenshots. Reference machine only; writes nothing.
	...
	( cd www && pipenv run python -m http.server $(PORT) > /dev/null 2>&1 ) &
	sleep 2
	PORT=$(PORT) CHECK=1 SHOT="$(SHOT)" pipenv run python3 -u tests/screenshots.py
	$(KILL_SERVER)
...
run-single-test: init lint | tests/test_pam.py
	...
	-google-chrome --version && chromedriver --version && chromium-browser --version
	pipenv run python3 --version
	PORT=$(PORT) pipenv run python3 -m pytest -k test_pam_setup tests/test_chrome.py
...
# Written to a temporary file and moved into place, so the module is never
# visible half-written.
#
# The recipe truncates with '>' and then appends six times. A browser fetching
# /js/version.js during that window gets a partial module and every import of
# it fails — which matters now that `make test` and `make screenshots` can run
# concurrently on different ports, since both may trigger this rule. mv within
# a directory is atomic on POSIX: a reader sees either the old file or the new
# one, never a partial.
www/js/version.js: Makefile $(SRC_FILES)
	$(call hdr,"$@")
	echo '// DO NOT DELETE!' > $@.tmp
	echo '// This file is automatically generated by make.' >> $@.tmp
	echo "export const VERSION = '$$(cat VERSION | tr -d ' \n')'" >> $@.tmp
	echo "export const BOOTSTRAP_VERSION = '$(BS_VER)'" >> $@.tmp
	echo $$(git rev-parse --short HEAD | \
		tr -d '\n' | \
		xargs -I{} echo "export const COMMIT_ID = '{}'" ) >> $@.tmp
	echo $$(git show -s --format=%ci $$(git rev-parse --short HEAD | tr -d ' \n')) | \
		xargs -I{} echo "export const COMMIT_DATE = '{}'" >>$@.tmp
	echo $$(git rev-parse --abbrev-ref HEAD) | \
		xargs -I{} echo "export const COMMIT_BRANCH = '{}'" >>$@.tmp
	mv $@.tmp $@
	@cat -n $@
